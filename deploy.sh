...
# Variables
PROJECT_DIR="/var/www/webapp/codeqrapp"
...
cd $TEMP_DIR
git clone https://github.com/issouf7507-dev/codeqr.git .
...
if pm2 list | grep -q "codeqrapp"; then
    pm2 restart codeqrapp
else
    pm2 start npm --name "codeqrapp" -- start
fi
